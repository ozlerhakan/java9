...
---
Tanımladığımız kod parçalarını düzenleyemek::

Tek satırdan oluşan tanımlamaların tekrar düzenlenmesi komut satırı üzerinde problem olmamakta fakat birden fazla satırdan oluşan tanımlamalarda bu yeterli olmamakta. Bunun için `/edit` komutunu sunulmaktadır. `/edit` komutunu kullanarak direkt bir kod yapısını düzenleyebiliyoruz. Bunun için bir snippet id numarası ya da ona referans gosterilen isimlendirme üzerinden ilgili parçayı belirtmemiz gerekiyor. Ayrıca son kayıt altına alınan tanımı da parametre girmeden `/edit` komutuyla güncelleyebiliyoruz. Eğer kendimiz farklı bir editor daha öncesinden belirlemediysek varsayılan olarak belirlenen editor üzerinden düzenleme imkanımız oluyor. JShell konfigürasyonunu nasıl değiştirebileceğimizi de sonraki bölümlerde ele alacağız. Önceki oluşturduğumuz `sum` metodunu gelin düzenleyelim.
